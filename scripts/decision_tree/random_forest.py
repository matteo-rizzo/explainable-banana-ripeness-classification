...
def train_forest():
    # Load configuration
    train_config: Dict = load_yaml("params/networks/random_forest.yml")
    ...
    for rs in range(num_rand_states):
        ...
        random_forest = RandomForestClassifier(random_state=rs, **train_config["forest_params"])
        random_forest.fit(x_train, y_train)
        y_pred = random_forest.predict(x_test).tolist()
    ...
    return random_forest, color_feature_names, num_classes
...
def grid_search_best_params():
    ...
    for rs in range(num_rand_states):
        ...
        # Setup and train classifier
        grid_clf = GridSearchCV(RandomForestClassifier(), n_jobs=-1,
