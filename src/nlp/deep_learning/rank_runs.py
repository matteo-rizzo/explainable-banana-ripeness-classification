...
def rank_runs(path: str | Path, metric_name: str, top_k: int) -> None:
    ...
    gs_config: dict = load_yaml("src/nlp/params/deep_learning.yml")["grid_search_params"]
    ...
    all_results = pd.DataFrame(all_results, columns=["id", metric_name, "epochs", *gs_params]).set_index("id").sort_values(by=metric_name, ascending=False)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Script to quickly obtain ranking from Grid Search results. Will create a result file in {path}/gs_ranking.csv.")
    # parser.add_argument('--help', action='help', help=)
    parser.add_argument("path", type=str, help="Path to folder with Grid Search dumps")
    parser.add_argument("--top_k", "-k", nargs="?", type=int, default=10, help="Best k results to print to console (the rest will be in CSV file)")
    parser.add_argument("--metric", "-m", nargs="?", type=str, default="f1", help="Metric name to sort by, should reflect naming used in dumps")
