...
def read_input(submission_path, task):
    ...
    for runs_admitted in ['run1', 'run2', 'run3']:
        ...
        if (len(raw_data_path) == 1):
            try:
                raw_data_predicted[runs_admitted] = pd.read_csv(unzipped.open(raw_data_path[0]), sep="\t", header=None, index_col=None, names=raw_column_names,
        ...
        if ((task.lower() == "b")):
            ...
            if (len(raw_data_path) == 1):
                try:
                    synt_data_predicted[runs_admitted] = pd.read_csv(unzipped.open(synt_data_path[0]), sep="\t", header=None, index_col=None, names=synt_column_names,
